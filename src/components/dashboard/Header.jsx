...
const Header = () => {
    ...
    const [hasUnreadNotifications, setHasUnreadNotifications] = useState(true); // Initial state
        ...
        <div className="relative flex flex-row items-center justify-between w-full bg-offWhite p-4">
            ...
            <div className="flex flex-row items-center justify-start space-x-2 sm:space-x-3 md:space-x-4 px-2">
                <div className="relative">
                    <PiBell 
                        className="text-3xl cursor-pointer text-blue hover:text-blue-700 transition-colors" 
                        onClick={toggleNotifications}
                    />
                    {unreadCount > 0 && (
                        <span className="w-3 h-3 bg-red-500 rounded-full absolute top-0 right-0.5 animate-pulse"></span>
                    )}
                </div>
                <img 
                    ...
                    alt="profile" 
                    className="w-8 md:w-10 h-auto rounded-full object-cover cursor-pointer hover:ring-2 ring-blue transition-all" 
                />
